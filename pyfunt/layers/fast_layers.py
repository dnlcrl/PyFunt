...
def max_pool_forward_fast(x, pool_param):
    ...
    single_frame = H == pool_height and W == pool_width

    if single_frame or (same_size and tiles):
...
def max_pool_forward_reshape(x, pool_param):
    ...
    pool_height, pool_width = pool_param[
        'pool_height'], pool_param['pool_width']
    stride = pool_param['stride']
    assert pool_height == pool_width == stride or (
        H == pool_height and W == pool_width), 'Invalid pool params'
    assert H % pool_height == 0 or H == pool_height
    assert W % pool_height == 0 or W == pool_width
